...
function createPost(parent, args, context) {
    return new Promise((resolve, reject) => {
        getConnection(function (errCon, connection) {
            ...
            connection.query(`INSERT INTO posts (title, description) VALUES ('${args.title}', '${args.description}')`, (err, results) => {
                console.log(results)
                resolve({
                    data: {
                        title: args.title,
    ...
    if (context.currentUser) {
        return new Promise((resolve, reject) => {
            getConnection((errCon, connection) => {
                ...
                description = '${args.description}'
                WHERE id=${args.id};`, (err, result) => {
                    if (err) reject(err);
                    connection.query(`SELECT * FROM posts`, (selectErr, selectResults) => {
...
function deletePost(parent, args, context) {
    if (context.currentUser) {
        return new Promise((resolve, reject) => {
            getConnection((errCon, connection) => {
                if (errCon) reject(errCon);
                connection.query(`DELETE FROM posts WHERE id=${args.id}`, (err, result) => {
                    if (err) reject(err);
                    connection.query(`SELECT * FROM posts`, (selectErr, selectResults) => {
                        if (selectErr) reject(selectErr);
                        if (selectResults.length > 0) {
...
function register(parent, args, context) {
    return new Promise((resolve, reject) => {
        getConnection((errCon, connection) => {
            ...
            VALUES
             ('${args.username}', '${sha256(args.password)}', '${args.firstName}', '${args.lastName}', '${args.address}')`,
                (err, result) => {
                    if (err) reject(err);
                    connection.query(`SELECT * FROM users WHERE username='${args.username}'`, (selectErr, selectResults) => {
                        if (selectErr) reject(selectErr);
                        if (selectResults.length > 0) {
